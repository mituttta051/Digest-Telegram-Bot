...
async def get_channels_with_permissions(user_id: int) -> list[(str, str, str, str)]:
    ...
    """
    result = []
    for channel_id, name, main_l, additional_l, auto_digest, auto_digest_date, api_key, folder_id, system_prompt in get_channels():
        try:
            administrators = await bot.get_chat_administrators(channel_id)
            if user_id in list(map(lambda x: x.user.id, administrators)):
                result.append((channel_id, name, main_l, additional_l, auto_digest, auto_digest_date, api_key, folder_id, system_prompt))
